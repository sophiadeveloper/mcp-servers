...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `git-node` | ✅ | ❌ | ❌ | ❌ | ❌ | `Server` low-level (`setRequestHandler`) | ✅ forte (enum su tutti i tool) | ✅ richiesto | ❌ | ✅ buono (schema semplici/espliciti; nessun array complesso) | Buona base per host severi: schema compatti e stabili; nessun metadata moderno (resources/prompts/annotations). |
| `docs-node` | ✅ | ❌ | ❌ | ❌ | ❌ | `Server` low-level (`setRequestHandler`) | ✅ forte (tool condensati) | ❌ | ❌ | ✅ buono (array con `items`, oggetti annidati dichiarati) | Compatibile lato tooling classico; manca superficie MCP moderna (resources/prompts) che migliorerebbe UX in VS Code. |
| `office-node` | ✅ | ❌ | ❌ | ❌ | ❌ | `Server` low-level (`setRequestHandler`) | ✅ forte (`word/excel/pdf` con enum) | ❌ | ✅ (`pdf_document.export_text`) | ⚠️ discreto/buono (array annidati con `items`; schema ricchi ma da testare con client strict su payload tabellari) | Area piu' sensibile per Copilot/VS Code per la complessita' schema: opportuni smoke test dedicati su `tools/list`. |
